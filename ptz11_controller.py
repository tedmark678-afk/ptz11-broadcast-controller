...
def check_camera_reachable():
    try:
        ...
        logger.info(f"Camera ping: {'✓ REACHABLE' if reachable else '✗ UNREACHABLE'}")
        return reachable
...
def send_visca_command(payload_hex):
    with visca_lock:
        try:
            ...
            if packet is None:
                return False, f"Packet build failed"
            sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    ...
    while True:
        try:
            ...
            if not success:
                ...
                frame = cv2.Mat.zeros((360, 640, 3), cv2.CV_8UC3)
